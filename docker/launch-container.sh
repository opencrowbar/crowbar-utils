...
do
	...
	docker run -d -e ADMIN_SERVER=${ADMIN_SERVER} -e CROWBAR_INSTALL_KEY=${CROWBAR_INSTALL_KEY} -h container$x.crowbar.org  newgoliath/crowbar-workload /bin/sh -c '/root/register-container.sh; /usr/sbin/sshd -D'
done
